...
const AllBooks = async () => {
    // Fetch data from database (Functionality remains 100% same)
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="mt-1.5 text-xs md:text-sm text-slate-300 font-medium max-w-2xl leading-relaxed">
                            Centralized repository control. Overview catalog entries, monitor storage parameters, and handle global item availability seamlessly.
                        </p>
                    </div>
                    
                    {/* টোটাল বুক কাউন্টার উইজেট */}
                    <div className="flex items-center gap-3.5 bg-white/5 backdrop-blur-xl rounded-2xl p-3.5 border border-white/10 self-start md:self-auto shadow-inner">
                        <div className="w-9 h-9 rounded-xl bg-purple-500/20 flex items-center justify-center text-purple-300 border border-purple-500/30">
                            <FiBookOpen className="animate-pulse" size={18} />
                        </div>
                        <div className="text-left">
                            <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Global Inventory</p>
                            <p className="text-xs font-bold text-purple-300">
                                {manageBooks?.length || 0} Records Live
                            </p>
                        </div>
                    </div>
                </div>
